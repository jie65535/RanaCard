    ...
    <el-tabs v-model="tab">
      <el-tab-pane label="可视化" name="visual">
        ...
        <template v-else>
          ...
          <div class="row"><el-button @click="applyBeginTemplate('vip')">模板：-80金币 并获得会员卡</el-button></div>
        </template>
        <div class="preview">
...
import SentenceCard, { type Sentence } from './SentenceCard.vue'
import BeginEffectCard, { type BeginCmd } from './BeginEffectCard.vue'

type Kind = 'card'|'pendant'|'begineffect'
const props = defineProps<{ modelValue: string, kind?: Kind }>()
const emit = defineEmits<{ (e:'update:modelValue', v:string): void }>()

const kind = computed(() => props.kind ?? 'card')
const tab = ref<'visual'|'dsl'>('visual')
const dslText = ref(props.modelValue || '')
const sentences = reactive<Sentence[]>([])
// 使用 ref 以便 v-model 能正确赋值（避免 const 变量被模板赋值失败）
const beginCmds = ref<BeginCmd[]>([])

const preview = computed(() => kind.value !== 'begineffect' ? serializeSentences() : serializeBegin())
...
function addSentence(){ sentences.push({ trigger:'Play', segments:[{ cond:{}, actions:[] }], consume:false, foresee:false }); dirtyFromVisual.value = true; emitFromVisual() }
function removeSentence(i:number){ sentences.splice(i,1); dirtyFromVisual.value = true; emitFromVisual() }
function moveUp(i:number){ if (i>0) { sentences.splice(i-1,2,sentences[i], sentences[i-1]); dirtyFromVisual.value = true; emitFromVisual() } }
function moveDown(i:number){ if (i<sentences.length-1) { sentences.splice(i,2,sentences[i+1], sentences[i]); dirtyFromVisual.value = true; emitFromVisual() } }

function onSentenceUpdate(i: number, val: any){
  // 替换数组项，确保父级拿到子组件变更
  sentences.splice(i, 1, val)
...
function applyFromTemplate(name:'grow1'|'money3'){
  if(name==='grow1'){
    sentences.splice(0)
    sentences.push({ trigger:'Play', segments:[{ cond:{}, actions:[{ type:'func', name:'RandomGrow', args:[{ label:'数量', value:'1' }] }] }], consume:true, foresee:false })
  } else {
    sentences.splice(0)
    sentences.push({ trigger:'RoundEnd', segments:[{ cond:{}, actions:[{ type:'attr', target:'Global', attr:'Money', mode:'add', value:'3' }] }], consume:false, foresee:false })
  }
}
...
function serializeSentence(s: Sentence): string {
...
function serializeSentences(): string { return sentences.map(serializeSentence).join(' # ') }
function serializeBegin(): string { return beginCmds.value.map((b:any)=> b.kind==='GlobalAdd' ? `Global,${b.attr},${b.value}` : b.kind==='OpenPack' ? `OpenPack(${b.packType};${b.size};${b.n})` : b.kind==='AddPendant' ? `AddPendant(${b.name})` : b.kind==='OpenOneOfFourteen' ? `OpenOneOfFourteen()` : b.kind==='AddOriginPendant' ? `AddOriginPendant()` : '').filter(Boolean).join(' # ') }

function initFromDSL(){
  suppressEmit.value = true
  try {
    sentences.splice(0); beginCmds.value.splice(0)
    const raw=(props.modelValue||'').trim(); dslText.value=raw
    if(!raw) return
    if(kind.value==='begineffect'){ parseBegin(raw); return }
    parseEffect(raw)
  } finally {
...
function parseEffect(raw:string){ const parts=raw.split('#').map(s=>s.trim()).filter(Boolean); for(const p of parts){ const m=p.match(/^(.*?)\s*<\s*(.*)\s*>\s*(\((.+)\))?\s*$/); if(!m){ sentences.push({ trigger:'Play', segments:[{ cond:{}, actions:[] }], consume:false, foresee:false }); continue } let triggerText=(m[1]||'').trim()||'Play'; const inner=m[2]||''; const tagStr=(m[4]||''); const s: Sentence={ trigger: triggerText, segments:[{ cond:{}, actions:[] }], consume:/\bConsume\b/.test(tagStr), foresee:/\bForesee\b/.test(tagStr) }; // handle Watch(...)
    const watchM = triggerText.match(/^Watch\((.*)\)$/)
    if (watchM) { s.trigger = 'Watch'; s.triggerArgs = watchM[1] }
    let current=s.segments[0]; const tokens=Array.from(inner.matchAll(/\{([^{}]+)\}|\[([^\[\]]+)\]/g)); for(const t of tokens){ if(t[1]){ current={ cond:parseCond(t[1]), actions:[] }; s.segments.push(current) } else if(t[2]){ current.actions.push(parseAction(t[2])) } } if(s.segments.length>1 && s.segments[0].actions.length===0 && Object.keys(s.segments[0].cond).length===0){ s.segments.shift() } if(s.segments.length===0){ s.segments.push({ cond:{}, actions:[] }) } sentences.push(s) } }
...
watch(() => props.modelValue, (v) => {
  ...
  if (!incoming) { dslText.value = ''; sentences.splice(0); beginCmds.value.splice(0); return }
  if (incoming === lastEmitted.value) return
  if (incoming === (dslText.value || '').trim()) return
